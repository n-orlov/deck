# Task 206 — phase3g-findings.md's F36/F38/F40/F41 Phase 3h disposition correction, evidence

Steering 001 / operator amendment rejected the F36/F38/F40/F41 disposition sentences in
`docs/reports/phase3g-findings.md` for the same false-disposition-prose ground as the R76 row
(task 205): they claimed the three forward-reverts restore 'unrelated' scenario shapes and
'leave the narrowed repair untouched' / 'do not touch it'. That is false against the diffs —
`afa55b8` IS the revert that restores the narrow repair in `internal/service/reconcile.go`,
`f700025` changes `features/status_probe.feature`'s stale-sampling scenario, and `2f952da`
changes `features/status_claude_hooks.feature`'s live-pane `StopFailure` assertion. This report
quotes each per-path diff so the four corrected sentences are grounded in the actual diff.
...
## SPEC.md is untouched by all three reverts (backs F41's surviving claim)

`git show f700025 -- SPEC.md` (expect empty):
```
```
(0 lines)

`git show 2f952da -- SPEC.md` (expect empty):
```
```
(0 lines)

`git show afa55b8 -- SPEC.md` (expect empty):
```
```
(0 lines)
...
`grep -c 'Phase 3h disposition' docs/reports/phase3g-findings.md` (expect `4`):
```
4
```

`awk -F'\\| Phase 3h disposition' '/^\| F(36|38|40|41) /{print $2}' docs/reports/phase3g-findings.md | grep -c -e unrelated -e 'do not touch' -e 'does not touch'` (expect `0`):
```
0
```

`grep -c 4b1d4dc docs/reports/phase3g-findings.md` (final code sha citation, expect >= 1):
```
4
```

`git diff --numstat 08171ce^ HEAD -- docs/reports/phase3g-findings.md` (expect exactly `4\t4`):
```
4	4	docs/reports/phase3g-findings.md
```

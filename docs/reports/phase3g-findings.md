...
`prds/phase3g-residuals-and-suite-determinism.md` got wrong, and the
...
| F21 | `internal/theme/contrast_test.go`'s floor does not yet cover `hint`/`surface`, `key`/`surface`, `error`/`surface`, or the text tokens over `Selection`/`SelectionIdle` that R82's now-themed dialogs actually draw | `internal/theme/contrast_test.go` (state of record `7033e12`; no sha in `1cfbd5a..050ca9f` touches the file) | task 023 (R84) is still `pending` in `tasks.json` as of this writing — this is an unstarted deliverable, not an abandoned one, and is tracked at the task level rather than argued here. Listed for completeness because [`phase3g.md`'s R84 section](phase3g.md#r84--the-contrast-floor-covers-the-pairs-a-dialog-actually-uses-task-023-not-yet-done) and its per-requirement table already carry it as "not done"; this row exists so a reader of *this* file sees the same fact without cross-referencing. |
...
`prds/phase3g-residuals-and-suite-determinism.md` is a protected path for this

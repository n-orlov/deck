...
- Task 021's residual (rename dialog's focused field has no `theme.Selection`
  ...
  not a contradiction in the requirement itself — the fix is known, only not
  yet landed. Recorded as [F18](#3-defects-found-and-deliberately-not-fixed-and-why) below,
...
- So R85's *own* work landed cleanly and met its own criterion; the breakage
  ...
  written. This is expected to make task 040 (whole-suite green) fail until
  a follow-up task finishes the convergence — see

...
| F37 | **This approach's own baseline (task 804) found `features/sort_order.feature` red under load.** The file's six raw `has status "error"` writes onto live shell panes (the sites F36 lists) raced F36's same unconditional repair: nothing in the scenario waited for the repair to land before reading the order table, so a repair that landed before the assertion ran reordered a promoted `running` row ahead of a genuinely-older `running` row in the attention tier. Reproducing it needs the repair to win the race deterministically, which never happens on an unforced run — the order assertion always outruns the 250ms reconcile tick — so it was forced in a throwaway `git worktree` at the immediately pre-fix commit, with one extra *existing* step (`after one configured reconcile interval deck client "A" screen still contains "waiting"`) inserted ahead of the order table, no other line touched; the worktree edit was never committed (`git worktree remove --force` + `git worktree prune`; `git status --porcelain` empty throughout). Failure text: `step error: deck client renders session "ord-alpha" at line 5, want strictly after session "ord-bravo" at line 7` | `features/sort_order.feature:35,63,91,119,149,223` (state of record `bedb65a`, immediately pre-fix) | **Fixed by task 804, `46dad5e`.** Standalone reproduction (from the pre-fix commit, worktree required because the race needs to be forced):<br>`git worktree add --detach .scratch-804 bedb65a` (then insert the one extra wait step ahead of the order table in the attention scenario)<br>`ci/run.sh sh -c 'cd .scratch-804 && env DECK_GODOG_PATHS=sort_order.feature go test ./features/ -run TestFeatures -count=1'`<br>→ exit 1, captured verbatim at `docs/reports/phase3g-804-sort-order-error-route/pre-change-red.log` / `.exitstatus`. The fix replaces every raw `error` write with a genuine nonzero pane exit (`shell session "X" exits with status 1`), routed through reconcile's crash-collection path rather than the live-pane repair path, durably out of the repair's reach; verified with five consecutive green runs at `46dad5e` (`docs/reports/phase3g-804-sort-order-error-route/run1.log`..`run5.log`, all exit 0). No `screen shows sessions in this order` table and no R53 selection-preservation assertion changed a line (`git diff features/sort_order.feature \| grep -E '^[+-].*\|'` empty, per task 804's own criterion). |
...
**Restated per this run's standing rules: F2 (`TestGoldenMinimumFrame` settle flake), F20 (the `status_recovery.feature` "dup pane" scenario's dup-pane/R76 interaction, already historically resolved by an earlier approach's task 040 per F20's own row above), F22 (`internal/interactive`'s `TestSessionRendersAreCoalescedAgainstAKnownByteArrivalPattern` flake) and F31 (the `attention_sort.feature` lost update) remain open and were untouched by this approach (tasks 801-808).** None of the six commits this approach produced (`794313f`, `bedb65a`, `46dad5e`, `4651653`, `41ae9cd`, `b7a3a81`, `b434079`, `33e7935`, `fdf4507`) touches `features/golden_frame_test.go`, `internal/interactive/render_test.go`, or `features/attention_sort.feature` (`git log --oneline 794313f^..HEAD -- features/golden_frame_test.go internal/interactive/render_test.go features/attention_sort.feature` is empty), and task 805's work on `features/status_recovery.feature` touched only the "stale tmux verdict" scenario ([F37](#3-defects-found-and-deliberately-not-fixed-and-why)-adjacent but distinct — see task 805's own README), never the "dup pane" scenario (`features/status_recovery.feature:23-51`) F20's row names. None of the four is claimed fixed by this approach; a recurrence of any of them belongs in a later approach's own report with its log path, not in a rewrite of this row.

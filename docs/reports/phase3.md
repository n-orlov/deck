...
|---|---|---|---|
| 1 | `DECK_UNDO_MS`/`DECK_DELETE_GRACE_MS` knobs, documented, findings | PARTIAL | Parsed and defaulted: `internal/config/config.go:140,144`, asserted by `internal/config/config_test.go` (`DECK_UNDO_MS`/`DECK_DELETE_GRACE_MS` cases incl. malformed-value rejection at line 377). Help lines deliberately **removed** (not yet added back) by commit `b941114` (task 059) until the undo/tombstone behaviour they'd describe actually ships — see `docs/reports/phase3-findings.md`. Documentation-in-help and the findings-for-the-operator half are task 102's. |
| 2 | Both windows tick under frozen `DECK_CLOCK` | PENDING | Task 106. |
...
| 23 | `dd` two-key delete: pending-`d`, confirm, tombstone, 60s undo, reap | PENDING | Store foundation landed (task 104): `internal/store.SoftDeleteSession`/`RestoreSession`/`ReapSession` (`internal/store/store.go`), `ListDeletedSessions` as the reaper's separate accessor, `ListSessions` excludes tombstoned rows by default; covered by `internal/store/tombstone_test.go` (`TestSoftDeleteSessionTombstonesAndHidesFromListSessions`, `TestRestoreSessionClearsTombstoneAndReturnsToListSessions`, `TestReapSessionRequiresTombstoneAndCascadesEvents`, `TestReapSessionRejectsMissingSession`, `TestSoftDeleteAndRestoreRejectMissingSession`). Task 105 lands the `dd` key/UI: `internal/service.Service.Delete` (`internal/service/delete.go`, covered by `internal/service/delete_test.go`) kills the live pane then soft-deletes; `internal/tui.Model`'s `pendingDelete`/`deleteConfirming` (`internal/tui/tui.go`) render the pending indicator and the confirm dialog naming the surviving conversation id and cwd, joining every open-dialog gating list and the §11.4 mouse/width-clamp contract; help overlay gained its `dd` line with both guard-word lists updated. Covered by `features/kill_delete_undo.feature`'s 7 new scenarios (`features/kill_delete_undo_test.go`), run via `DECK_GODOG_TAGS="@requirement-22-undo-toast && ~@real-agents"`. Still PENDING overall — task 106 (60s undo window + reap loop) remains. |

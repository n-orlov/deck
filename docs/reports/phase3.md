...
| 26 | Purge offered only in delete confirm, names exact path, declines honestly | PARTIAL | Task 109: `internal/agent`'s `Adapter` interface gains `TranscriptPaths(in TranscriptInput) (path string, ok bool)`, and `Caps` gains `HasTranscript bool`, as a declared capability (never an inferred path) -- `Claude`/`Pi` implement exactly the convention recorded in `docs/reports/phase3-findings.md`'s "Transcript path/naming convention, per adapter" section, `Shell` declines outright (`HasTranscript=false`). Go tests: `internal/agent/transcript_test.go`'s `TestClaudeTranscriptPathFindsRealFile`/`TestClaudeTranscriptPathMissingHomeDegrades`/`TestClaudeTranscriptPathNoMatchingFileDegrades`, `TestPiTranscriptPathFindsRealFile`/`TestPiTranscriptPathMissingHomeDegrades`/`TestPiTranscriptPathNoMatchingFileDegrades`, and `TestShellTranscriptPathAlwaysDeclines`. `grep -rn TranscriptPaths internal/` now finds the declaration and both adapters' implementations (previously empty per the reviewer's own check). `grep -rn 'ReadDir\|filepath.Glob' internal/ --include=*.go` shows the one glob this convention requires (`internal/agent/pi.go`, inside the declared capability itself) and nothing outside `internal/agent` infers or globs a transcript directory. The purge action itself (offered only in the delete confirm, naming the exact path, deleting only that declared file) is task 110 -- this row stays PARTIAL until that lands. |
...
| 29 | R1: cwd fingerprint across every destructive path + every undo | PARTIAL | Task 108: `features/kill_delete_undo.feature` `@requirement-29-kill`/`@requirement-29-kill-undo`/`@requirement-29-delete`/`@requirement-29-delete-undo`/`@requirement-29-reap` (5 scenarios, each seeding an adversarial cwd — a `state.db`-named file, a dotfile, a subdirectory, a mode-0444 read-only file — via `a scratch directory ... is seeded with:`, then asserting `the directory ... still matches fingerprint ...` after kill, kill+undo, dd, dd+undo and dd+reap respectively). Real output: `docs/reports/phase3-task108-scenarios.log` (5/5 scenarios, 45/45 steps, green). Task 113 still owes purge/archive/bulk-action coverage of the same fingerprint. |

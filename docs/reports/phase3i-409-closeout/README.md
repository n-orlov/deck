...
|---|---|---|
| R98 | 401 (`96bff5682571511a3cc63b1065d593fe64440ae9`), 402 (`3b70bfbc7e3552ff375ae675af117805a1eee944`), inheriting 104 (`a65190e`, `0177991`), 105 (`e31c37e`), 106 (`8d1b41a`), 107 (`17e185c`), 108 (`7e141ad`, `a063296`), 109 (`8e5e036`, `55c0818`), 201 (`c025c54`), 202 (`5b554f2`) from earlier approaches | `F` skips the two named contention refusals (attached-client, live-ownership) and is now bound, in the keymap and in the `?` overlay while being **out of** `footerLegend`'s curated fixed set per SPEC §11.3 — task 401 removed the row, task 402 pinned the resulting glyph set as a closed list against SPEC §11.3's own sentence so it cannot drift back |
| R99 | inherited unchanged from tasks 101 (`eb1e917`), 102 (`244f66f`), 103 (`a05467b`), 121 (`d7f149c`) (approach 1; no approach-4 task touches this requirement) | Single-shot force claim: exactly one winner, every loser stands down with no error (`internal/tmux/ownership.go`, `internal/tmux/force_ownership_test.go`, the `↵` refused/`F` wins/A is told feature scenario) |
| R100 | inherited unchanged from tasks 110 (`a790061`), 111 (`e6157d8`), 112 (`4ef6d28`, `c525b35`, `cbc6692`), 113 (`625c663`), 114 (`cb27c5c`), 115 (`1b71c74`), 203 (`a559e7c`), 304 (`ca00cb6`) (approaches 1-3; no approach-4 task touches this requirement) | The original geometry survives an arbitrary chain of steals via the durable window option, and finding 5's correction (task 304, `ca00cb6`) retracted the earlier misquote that had invented an R100/SPEC disagreement — R100 and SPEC 11.9 agree, both asking for a plain unconditional unset |
| R101 | inherited unchanged from tasks 116 (`4ac5970`), 117 (`c95e848`), 118 (`be7a387`), 120 (`5fb9e4f`, `e398881`), 136 (`fcdb994`), 121 (`d7f149c`), 122 (`8d0437f`), 123 (`704fd1a`) (approaches 1-2; no approach-4 task touches this requirement) | The displaced client falls into the lost-attach dialog with its keyboard silenced; task 136 (not 119, which is `failed` — see below) proves the stolen flavour's teardown never reaches `ownership.Release` |
| R102 | inherited unchanged from tasks 124 (`7371c07`), 125 (`83e92d5`), 126 (`b9243a1`) (approach 1; no approach-4 task touches this requirement) | Passive `previewFit` stands down under a foreign live claim, so a second client merely selecting a row no longer resizes a window it doesn't hold |
...
Every row above names each discharging task **with the commit that carries it**, task id first and
its sha in parentheses; where a task landed more than one commit all of them are named. Each sha
is a real commit in this repository (`git cat-file -e <sha>^{commit}` succeeds for every one; the
citation guard re-checks them all at the pushed HEAD, see the last section). The inherited rows
name earlier-approach commits because no approach-4 task touches those requirements — approach 4
lands exactly two code commits, 401's `96bff56` and 402's `3b70bfb`, both in R98's row.

...
## Approach 06/07 close: the R110 record-staleness cure (tasks 103, 097, 098, 141)

Approach 04's independent review found R110's record re-presented stale claims after each
targeted line fix (approaches 03, 04 and 05 each cured only the sentence its own review pass
named, then were rejected on a different stale sentence the next pass). Operator steer 003 ordered
a single systematic audit in place of further per-line fixes, and steer 002 separately ordered a
record of why approaches 03-05 were rejected on the petition-re-file findings. The four commits
below, in landing order, discharged both:

- **Task 103** — `f01f2f95e5962ffeb1cd5cbbfd0d47f33b14150b` — ran a case-insensitive marker grep
  for present-tense staleness phrases (the expression this document's Standing Rules name)
  separately over `docs/reports/phase3j.md`, `docs/reports/phase3j-findings.md` and the
  DELIVERY-LOG Phase 3j paragraph, recorded a verdict for every hit rather than only the two
  review had named, and rewrote each hit that had actually gone stale in the past tense naming the
  commit that made it history.
- **Task 097** — `6acf6cafd48af03cb649f27dcaf70890fd56969f` — fixed `docs/reports/phase3j-findings.md`'s
  §7 table-of-contents entry for §8, which had gone stale by naming tasks 058-060 instead of the
  081-083 re-run that superseded them, and fixed the matching anchor link.
- **Task 098** — `71f6f2f9fc994fc8fcb810cadd7d43e34d52f4ac` — added `docs/reports/phase3j-findings.md`
  §12, recording that approaches 03, 04 and 05 were each rejected solely on the two petition
  re-file findings (the Task 204 whole-suite obligation and the create/resume context obligation),
  that the underlying cause was the review engine's own gap — it writes CONFIRMED petition
  adjudications back to the record but never REFUTED ones, so a REFUTED petition kept getting
  re-presented to review as open — and that the operator closed both terminally REFUTED
  adjudications directly in the run's own untracked petitions record, citing operator steers 002
  and 003 in prose rather than a run-state path.
- **Task 141** — `f0bedf6a3141efa6c8f5dd4244878031da18d6af` — rewrote the nine present-tense
  clauses inside that same new §12 in the past tense, curing the one residual task 098's own
  validation found: §12's own narration of the closure had used present-tense phrasing that itself
  became a marker hit once the section existed, so a follow-up commit restated the closure as
  something that had happened.

Together these four commits are what the R110 row above now cites for the record-staleness
residual: a repeatable systematic audit in place of per-line spot fixes, the findings-file table
of contents corrected to match its current §8, the petition-rejection history recorded once, and
that same record's own present-tense language corrected in a second pass.

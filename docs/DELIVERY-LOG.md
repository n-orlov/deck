...
| 4b | Manual session groups & the interactive scroll cue | `prds/phase4b-manual-groups-and-scroll-cue.md` | blob c3c251a | `deck-phase4b` | 2026-09-19 07:40Z — the run's own first start (`engine.started` + `state: running, resumed: false`, `events.jsonl` ids 3-4, 2026-09-19T07:40:28Z). An operator abort at 12:03:39Z (`state: aborted`, id 272) and the `resumed: true` restart at 12:03:58Z (ids 275-276, which is also what this run's `status.json` `startedAt` now holds) are a restart of this same run, not its start | in progress — task 027 (guard re-verify at the landing sha) is the only task left in the plan at the time this row is written | `in progress` on approach 1 of 3 — tasks 001-026 landed, freeze line held since task 021's launch (0806ba6), no `failed` task | **not yet operator-verified** — per-requirement record in [`docs/reports/phase4b.md`](reports/phase4b.md) (task 024, attribution corrected in `6c2b95c`), findings in [`docs/reports/phase4b-findings.md`](reports/phase4b-findings.md) (task 025), gate/stability headlines in [`docs/reports/phase4b-final-suite/README.md`](reports/phase4b-final-suite/README.md) and [`docs/reports/phase4b-stability10/README.md`](reports/phase4b-stability10/README.md) |

...
| 4b | Manual session groups & the interactive scroll cue | `prds/phase4b-manual-groups-and-scroll-cue.md` | blob c3c251a | `deck-phase4b` | 2026-09-19 07:40Z — the run's own first start (`engine.started` + `state: running, resumed: false`, `events.jsonl` ids 3-4, 2026-09-19T07:40:28Z). An operator abort at 12:03:39Z (`state: aborted`, id 272) and the `resumed: true` restart at 12:03:58Z (ids 275-276, which is also what this run's `status.json` `startedAt` now holds) are a restart of this same run, not its start | not completed yet — approach 2 of 3, task 012 (guard re-verification at the tree this approach ends on) is the only task left in the plan at the time this row is written | `in progress` on approach 2 of 3 — approach 1 (tasks 001-026, final code sha `f13c848`) was rejected by review on three blocking findings plus one residual (`/run/ralphd/approaches/01/review-findings.json`): B1, the list filter matched the raw (empty) group key instead of the sidebar's own displayed default label, so `/default` hid every default-group session; B2, an already-open settings Groups panel did not pick up another client's rename/create/delete on the panel's own next ordinary reload; B3, once pipe displacement's periodic fallback reseeded the interactive preview, the grid still carried artificial scrollback and rendered a scrolled-back cue over what should read as live; R1, the `c` help line still named the removed workspace-group model. Approach 2 cures all four in tasks 001-004, attests the cures against the four named regression tests (`docs/reports/phase4b-a2-cures/README.md`, task 006, commit `d0b3100`), pins the cured revision at `70c7430` (task 005) and re-sweeps it clean (tasks 007-008). Task tally at this row's own write time (`/run/ralphd/tasks.json`): 10 of 12 tasks `validated` (001-010), task 011 (this row) `in-progress`, task 012 `pending` — no `failed` task | **not yet operator-verified** — per-requirement record in [`docs/reports/phase4b.md`](reports/phase4b.md) (task 009, re-written from scratch against the approach-2 pinned revision `70c7430`), findings in [`docs/reports/phase4b-findings.md`](reports/phase4b-findings.md) (task 010's §5 approach-2 addendum), gate headline in [`docs/reports/phase4b-a2-final-suite/README.md`](reports/phase4b-a2-final-suite/README.md) (task 007, exit 0, 445s) and stability headline in [`docs/reports/phase4b-a2-stability10/README.md`](reports/phase4b-a2-stability10/README.md) (task 008, 10/10 passed) |
| 4c | Group navigation and order | `prds/phase4c-group-navigation-and-order.md` | blob 3abd517 | `deck-phase4c` | 2026-09-23 04:57Z — the run's own first start (`engine.started` + `state`, `events.jsonl` ids 3-4). Two later `engine.started`/`state` pairs at 11:16:01Z and 12:50:30Z (ids 337-338, 405-406) are restarts of this same run, not its start | not completed yet — cure wave in progress; six `retake-01-*` re-verification tasks remain pending at the time this row is written | `in progress` on approach 1 of 4 — approach 1's original landing (tasks 001-022, final code sha `2bb61a8`… superseded, see below) was rejected by review (iteration 64, `/run/ralphd/review-findings.json`) on five blocking product findings (F1 marked x/dd and detail r/l not unconditionally inert on a header; F2 an empty or structural-default group with zero sessions could not be folded; F3 a header cursor painted no visible selection cue; F6 the re-sort/re-group seam could strand the cursor off-screen or on a hidden stop; F7 folding the interactive session's own group dropped its name from the live preview border), three evidence findings after an operator-directed F5 split (F4 missing ten-run stability evidence; F5a/F5b/F5c missing phase report/findings+DELIVERY-LOG row/final-code guards), and one residual (F8 probe-report wording). Operator ruling 001 (curing in place, not archiving) plus steers 002-004 directed the order: `cure-01-01`..`cure-01-05` fix F1/F3/F7/F2/F6 in place (`cdce264`, `e23bc49`, `c864e6b`+`5da35c5`, `3529eb9`, `4e30475`), `022` re-runs the unnarrowed whole-suite gate at the resulting final code sha `1ad530b` (two further sweep-found escapes cured in the same task, `340b4d6`+`1ad530b`), `cure-01-06` supplies the ten-run stability sweep, `cure-01-07`/`cure-01-08`/`cure-01-09` supply the phase report/findings+row/guards, and `cure-01-10` cures F8. Task tally at this row's own write time (`/run/ralphd/tasks.json`, 33 tasks): 26 `validated`, `cure-01-08` (this row) `in-progress`, 6 `retake-01-*` re-verification tasks `pending` — no `failed` task. Tasks 017-021 are deleted rows (steer 003): 017 was unsatisfiable on a stale package count and its actual gate scope is discharged by `022`; 018-021's scope is carried independently by `cure-01-06`/`-07`/`-08`/`-09` | **not yet operator-verified** — per-requirement record in [`docs/reports/phase4c.md`](reports/phase4c.md) (cure-01-07), findings in [`docs/reports/phase4c-findings.md`](reports/phase4c-findings.md) (this row, cure-01-08), gate headline in [`docs/reports/phase4c-fullsuite/README.md`](reports/phase4c-fullsuite/README.md) (task 022, exit 0, 19/19 packages), stability headline in [`docs/reports/phase4c-stability10/README.md`](reports/phase4c-stability10/README.md) (cure-01-06, 10/10 passed) and final-code guards in [`docs/reports/phase4c-guards/README.md`](reports/phase4c-guards/README.md) (cure-01-09) |

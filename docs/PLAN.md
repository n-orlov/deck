...
7. **Phase 3b sits between 3 and 4, not in Phase 7.** It goes **after Phase 3** because it
   needs Phase 3's `mouse on` server option and its `tmux_mouse` knob (requirements 48–50),
   and because Phase 3's destructive paths must be settled before a mode that resizes panes
   lands on top of them. It goes **before Phase 4** at the operator's instruction (22 Aug
   2026), which reorders the §14.2 Codex spike behind it; Phase 4 stays uncut. It **shrinks
   Phase 7**: `SPEC.md` §11.1's send-without-attach exists because you cannot see what you are
   typing into, and this mode removes that premise, so §11.1 is now marked superseded rather

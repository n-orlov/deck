...
func TestNavigationVisitsEveryVisualRow(t *testing.T) {
	...
	// ↓ ("j"): from the first visual row, n-1 successive presses must
	// visit every remaining visual row EXACTLY ONCE, forward, never
	...
	if !intsEqual(visitedDown, wantVisual) {
	...
	m = press(m, runeKey("j"))
	if m.selected != last {
		t.Fatalf("↓ at the last visual row moved to %d, want to stay at %d", m.selected, last)
	}

	// ↑ ("k") must retrace the exact same path in reverse.
	wantUp := make([]int, n)
	for i, idx := range wantVisual {
		wantUp[n-1-i] = idx
	}
	...
	m = press(m, runeKey("k"))
	if m.selected != first {
		t.Fatalf("↑ at the first visual row moved to %d, want to stay at %d", m.selected, first)
	}
	...
	if want := wantVisual[0]; m.selected != want {
		t.Fatalf("g landed on %d, want %d (first visual row)", m.selected, want)
	}
	m.selected = wantVisual[n/2]
	m = press(m, runeKey("G"))
	if want := wantVisual[n-1]; m.selected != want {
		t.Fatalf("G landed on %d, want %d (last visual row)", m.selected, want)
	}
	...
	// never revisiting, exactly like ↓ but with wraparound
	// (nextAttentionSelection, internal/tui/tui.go, walks
	// m.visualOrder()).
	m.selected = wantVisual[0]
	...
	if !intsEqual(visitedSpace, wantVisual) {
		t.Fatalf("space visited %v, want %v", visitedSpace, wantVisual)
	}
	m = press(m, tea.KeyMsg{Type: tea.KeySpace})
	if want := wantVisual[0]; m.selected != want {
		t.Fatalf("space after visiting every row wrapped to %d, want %d (first visual row)", m.selected, want)
	}
}

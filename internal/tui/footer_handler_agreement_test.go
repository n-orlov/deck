...
// function a footer action key can reach, shared across all seven key
// checks below so building one Model wires every handler at once: the row
// class fixtures below are shared across all seven keys, and a key that
// early-returns on a nil service function (m.kill == nil, m.resume == nil,
// ...) would silently read as "did not act" for the wrong reason if any
// one of these were left unwired.
type footerAgreementRecorder struct {
	killCalled, resumeCalled, restartCalled, unarchiveCalled, acknowledgeCalled bool
}
		...
		},
		{
			...
			build: func() (Model, *footerAgreementRecorder) {
				return buildFooterAgreementModel(
					store.Session{ID: "s1", Name: "alpha", Agent: "claude", Status: "running"}, nil, nil)
			},
		},
		{
			name: "stopped",
			build: func() (Model, *footerAgreementRecorder) {
				return buildFooterAgreementModel(
					store.Session{ID: "s1", Name: "alpha", Agent: "claude", Status: "stopped"}, nil, nil)
			},
		},
		{
			name: "archived",
			build: func() (Model, *footerAgreementRecorder) {
				return buildFooterAgreementModel(
					store.Session{ID: "s1", Name: "alpha", Agent: "claude", Status: "stopped", ArchivedAt: 100}, nil, nil)
			},
		},
		{
			...
			build: func() (Model, *footerAgreementRecorder) {
				return buildFooterAgreementModel(
					store.Session{ID: "s1", Name: "alpha", Agent: "claude", Status: "waiting", Acknowledged: false}, nil, nil)
			},
		},
		{
			// Selected row (s1) is live/killable/restartable/archivable on
			// its own; the marked set (s2) is not. Y/r/R/A/U's own handlers
			// (case "Y"/"r"/"R"/"A"/"U") never consult m.marked -- footer's
			// entries for them all pass batch=false too -- so they must act
			// on s1 (the selection) here, ignoring s2 entirely. x/dd DO
			// switch to the marked set once it is non-empty, so they must
			// NOT act here: s2 is the only marked row and it refuses both.
			name: "non-empty mark set: selected row differs from the marked row",
			build: func() (Model, *footerAgreementRecorder) {
				m, rec := buildFooterAgreementModel(
					store.Session{ID: "s1", Name: "alpha", Agent: "claude", Status: "running"},
					[]store.Session{{ID: "s2", Name: "beta", Agent: "claude", Status: "stopped"}},

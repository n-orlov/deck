...
// TestReconcileRepairsBareErrorRowWithLivePane is task 1101's re-widening of
// the live-pane repair (review finding 2, steering 003): SPEC.md:560-566
// draws no exception by source or by whether the row already carries a
// pane-exit verdict -- "A terminal status -- stopped, error -- claims there
// is nothing running here, and a live, non-dead pane is direct evidence
// against it." A hook- or probe-written `error` row with no pane_exit_status
// at all (never observed a crash, so it carries no verdict for a live pane
// to contradict) paired with a live, non-dead pane is exactly the same
// invariant violation as a `stopped` row or an `error` row that DOES carry a
// crash verdict -- the pane is the part that is right. Task 902 had narrowed
// the trigger to exclude this exact row (finding F40); that narrowing is
// reverted here per review finding 2 and steering 003, which hold F40's
// carve-out has no basis in SPEC.md and forbid hunting for a product-side
// exception.

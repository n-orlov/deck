...
//   - event_retention_days (task 332, steer 3e-001 §6.4/§7): the sole
//     consumer is cmd/deck/main.go's tuiReconcile closure, which calls
//     Store.EnforceEventRetention with a `settings` local captured once
//     before the Model exists -- the same shape as stale_after/tmux_mouse
//     above, and for the same reason: there is no path back into a
...
//   - post_destroy (task 012, phase3j): the sole consumer is

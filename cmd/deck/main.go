...
func main() {
	...
	model := tui.NewWithShellCreatorAttacherKillerResumerAndProfileSwitcher(db, settings, tui.TmuxHealth(settings), sessions.CreateShell, client.AttachCommand, sessions.Kill, sessions.Reconcile, sessions.Resume, sessions.SetPermissionProfile)

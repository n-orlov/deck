...
Feature: The `,` settings takeover (requirement 48)
  ...
  Scenario: deleting an empty group in settings removes it with no confirm prompt (R131 part 2)
    Given deck client "A" is started
    When deck client "A" sends ","
    And deck client "A" sends "j"
    And deck client "A" sends "j"
    ...
    When deck client "A" sends "	"
    And deck client "A" sends "n"
    And deck client "A" sends "empty-one"
    And deck client "A" sends ""
    Then deck client "A" screen contains "created group empty-one"
    When deck client "A" sends "d"
    Then deck client "A" screen contains "deleted empty group empty-one"
    And deck client "A" screen does not contain "moves them to default"
    And deck client "A" screen contains "no groups yet"
    When deck client "A" sends ""
    Then deck client "A" screen contains "deck - sessions"
    When deck client "A" exits cleanly

  Scenario: deleting a non-empty group in settings offers moving members to default or the same dd confirm (R131 part 2)
    Given deck client "A" is started
    When deck client "A" sends ","
    And deck client "A" sends "j"
    And deck client "A" sends "j"
    ...
    When deck client "A" sends "	"
    And deck client "A" sends "n"
    And deck client "A" sends "tidy-up"
    And deck client "A" sends ""
    Then deck client "A" screen contains "created group tidy-up"
    When deck client "A" sends ""
    Then deck client "A" screen contains "deck - sessions"
    When deck client "A" creates shell session "groups-delete-move" into group "tidy-up" with a fresh working directory labelled "groups-delete-move"
    And deck client "A" sends ","
    And deck client "A" sends "j"
    And deck client "A" sends "j"
    And deck client "A" sends "j"
    And deck client "A" sends "j"
    And deck client "A" sends "	"
    Then deck client "A" screen contains "tidy-up"
    When deck client "A" sends "d"
    Then deck client "A" screen contains "1 session(s)"
    And deck client "A" screen contains "moves them to default"
    When deck client "A" sends "m"
    Then deck client "A" screen contains "moved to default"
    When deck client "A" sends ""
    Then deck client "A" screen contains "deck - sessions"
    And deck client "A" screen does not contain "tidy-up"

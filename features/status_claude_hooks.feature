...
Feature: Claude hook status truth
  ...
  Scenario: Every declared Claude hook maps to honest status through both identity routes
    ...
    Then the state database session "hook truth" has hook status "running", reason "", message "", acknowledged 1, and notify_epoch 0
    ...
    And the state database session "hook truth" has hook status "idle", reason "", message "permission granted; work is complete", acknowledged 0, and notify_epoch 1
    ...
    When fake Claude session "hook truth" fires "Notification" for itself using conversation identity:
      | notification_type | permission_prompt |
    Then the state database session "hook truth" has hook status "waiting", reason "permission_prompt", message "permission granted; work is complete", acknowledged 0, and notify_epoch 1

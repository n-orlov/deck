...
Feature: Launch-lease race
  ...
  Scenario: three clients racing resume on one row produce exactly one launch
    ...
    Then exactly one of deck clients "A", "B" and "C" screen contains "starting elsewhere"
    And within one configured reconcile interval deck client "A" screen contains "starting"
    And within one configured reconcile interval deck client "B" screen contains "starting"
    And within one configured reconcile interval deck client "C" screen contains "starting"
